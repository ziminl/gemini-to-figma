...
데이터 구조: 
{"totalHeight": 전체길이, "elements": [
{"type": "TEXT" 또는 "RECT", "x": 숫자, "y": 숫자, "w": 숫자, "h": 숫자, "content": "내용", "color": "#HEX", "fontSize": 숫자}
]}

배치 가이드:
- 중앙 정렬 텍스트는 x값을 (1440 - 요소너비) / 2로 계산해라.
- 섹션 배경은 w: 1440으로 설정하고 x: 0에 배치해라.
- 요소들이 겹치지 않게 y값을 누적해서 계산해라.
...
섹션 구성:
1. 헤더 (로고 + 네비게이션: Home, Work, About, Contact)
2. 히어로 섹션 (메인 카피 + 서브 텍스트)
3. 포트폴리오 섹션 (프로젝트 6개)
4. About 섹션 (자기소개 텍스트)
5. Contact 섹션 (이메일 + 버튼)
6. 푸터 (카피라이트)
...
포트폴리오 섹션 상세:
- 총 6개의 프로젝트를 카드 형태로 배치
- 3열 그리드 (한 줄에 3개씩, 총 2줄)
- 각 카드 구성:
  - 이미지 영역 (RECT)
  - 프로젝트 제목 (TEXT)
  - 설명 (TEXT)
...
출력은 반드시 JSON 형식으로만 생성해라. 추가 설명은 절대 포함하지 마라.
```
